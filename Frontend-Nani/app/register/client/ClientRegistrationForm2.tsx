...
const CustomMap =
  ...
  }) || require("../../../components/CustomMap").default;
    ...
    if (Platform.OS === "web") {
      return (
        <View style={styles.webPickerContainer}>
          ...
          >
            <Text style={{ color: "#886BC1", fontWeight: "bold" }}>
              Aceptar
            </Text>
          </TouchableOpacity>
        </View>
    ...
    try {
      if (Platform.OS === "web") {
        ...
        if (data.length > 0) {
          ...
          Alert.alert(
            "No encontrado",
            "Prueba especificando: Colonia, Ciudad, Honduras",
          );
        }
      } else {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert(
          "Error",
          responseData.message || "No se pudo completar el perfil.",
        );
      }
    } catch (error: any) {
      ...
      Alert.alert(
        "Error de Conexión",
        "Verifica tu internet o el estado del servidor.",
      );
    } finally {
      setIsSubmitting(false);
    }
